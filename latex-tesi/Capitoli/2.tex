...
\newpage
\section{Creazione del circuito da wireshark}
Ci sono diversi elementi che possono indicare che un circuito onion è in creazione, innanzitutto esso viene creato sfruttando i protocolli tcp e tls\footnote{In particolare la nuova versione di Tor usa TLSv1.3}, inserendo quindi TLS come filtro possiamo vedere in Wireshark come viene generato un circuito.
\importImage{
    \label{fig:Wireshark_circuit_raw}
    \includegraphics[width=\textwidth]{Wireshark/circuit_raw}
    \caption{Wireshark circuit}
}\\
Da qui vediamo che i due IP principali a cui il dispositivo si connette sono \lstinline{65.21.52.121} e \lstinline{185.177.229.16}. Vediamo anche che Tor scambia con questi due ip lo stesso numero di pacchetti e lo stesso numero di byte, per cui capiamo che vi è una correlazione.
...
Vediamo quindi che come primo punto viene eseguito un TCP handshake, successivamente inizia lo scambio di chiavi tramite TLSv1.3, possiamo inoltre notare che la porta con cui comunichiamo con il server \lstinline{65.21.52.121} la 9001\footnote{Tor Project suggerisce di non usare la porta 9001 per i bridge relay perchè è facilmente associabile alla rete Onion\cite{OnionAvoid9001}}.
Ad ogni messaggio TLS corrisponde una risposta ACK TCP che possiamo omettere dal filtro.
\newpage
Analizziamo il primo pacchetto TLS che viene inviato al server, è denominato \textbf{Client Hello} e possiede vari campi
\begin{itemize}
    \item \textbf{Cipher Suites}, una lista di protocolli di criptografia simmetrica supportati dal client che invia la richiesta, come descritto nell'RFC 8448. In questo caso i protocolli supportati sono 18, tra cui abbiamo AES 128 GCM SHA256 e AES 256 GCM SHA384
    \item Compression Methods, implementato in TLSv1.3 solo per retrocompatibilità dei server, che possono anche ricevere e gestire richieste TLSv1.2. A differenza dei client TLSv1.3 che devono impostare questo parametro a 0(null), altrimenti riceveranno un \lstinline{illegal_parameter} message
    \item \textbf{Extensions}, usato sia dalle applicationi che sfruttano TLS, come onion, che da funzionalità aggiuntive di TLSv1.3, implementate come estensioni per mantenere la retrocompatibilità, cosi che i server legacy possano ignorare le estensioni non riconosciute
\end{itemize}
\cite{TLSv1_3}
\importImage{
    \label{fig:Client_Hello}
    \includegraphics[width=\textwidth]{Wireshark/Client_Hello}
    \caption{Onion Client Hello}
}
\newpage
...
\begin{itemize}
    ...
    \item \lstinline{supported_groups}, precedentemente chimato Supported Elliptic Curves Extensions, indica gli Elliptic Curves utilizzati ordinati in base alla preferenza del client \cite{RFC8422}
    \item \lstinline{session_ticket}, non usato
    \item \lstinline{encrypt_then_mac}, non usato
    \item \lstinline{extended_master_secret}, non viene più usato ma deve comunque essere indicato per retrocompatibilità\cite{RFC7627}
...
\importImage{
    ...
    \caption{Onion Server Hello}
}
% TODO aggiungi sessione wireshark per mostrare la connessione ad un sito onion
